...
// icons
import { PlusOutlined, ProfileOutlined, TeamOutlined, SolutionOutlined, FileImageOutlined, UserAddOutlined } from '@ant-design/icons-vue';
...
const items = shallowRef([
  ...
  { tab: 'Friend Request', icon: UserAddOutlined, href: '' },
]);
</script>
    ...
    <div class="pb-0">
      ...
      <v-row class="mt-4">
        <v-col lg="3" md="3" cols="12" class="text-center text-md-end">
          <div class="userImage"><img :src="UserImage" width="150" alt="profile" class="rounded-md" /></div
          ></v-col>
        <v-col lg="9" md="9" cols="12">
          <v-row class="me-0">
            <v-col md="5" cols="12" class="text-center text-md-start">
              <h5 class="text-h5">Mohammad Nadr</h5>
              <small class="opacity-50">Web Developer</small>
            </v-col>
            <v-col md="7" cols="12">
              <div class="text-center text-md-end">
                <v-btn color="secondary" variant="outlined" class="me-2">Message</v-btn>
            ...
            <v-col md="12">
              <v-tabs v-model="tab" color="primary" class="profiletab" scrollable>
                <v-tab v-for="item in items" :key="item.tab" :to="item.href">
                  <component :is="item.icon" :style="{ fontSize: '18px' }" class="me-2"></component>
                  {{ item.tab }}
                </v-tab>
              </v-tabs>
            </v-col>

      ...
      <v-card class="bg-surface mb-4" variant="outlined">
        <v-card-text class="pa-0">
          <v-tabs v-model="tab" color="primary" class="custom-tab" hide-slider>
            <v-tab value="tab-1" rounded="md" class="text-start">
              <v-avatar size="24" color="primary" variant="tonal" class="me-2"> 1 </v-avatar>
              Cart
            </v-tab>

            <v-tab value="tab-2" rounded="md" class="text-start" :disabled="store.cart.length < 1">
              <v-avatar size="24" color="primary" variant="tonal" class="me-2"> 2 </v-avatar>
              Shipping information
            </v-tab>

            <v-tab value="tab-3" rounded="md" class="text-start" :disabled="store.cart.length < 1">
        ...
        <v-window-item value="tab-2" class="pa-1">
          <v-row>
            ...
            <v-col md="4" cols="12">
              <PromoCode />
              <OrderInformation />
              <v-btn class="mt-6" block variant="flat" color="primary" @click="changeTab('tab-3')">Process to Checkout</v-btn>
            </v-col>
          </v-row>
        </v-window-item>
        <v-window-item value="tab-3" class="pa-1">
          <v-row>
            ...
            <v-col lg="3" cols="12" md="4">
              <PromoCode />
              <OrderInformation />
              <v-btn block variant="flat" color="primary" class="mt-4" @click="thankyou = true">Process to checkout</v-btn>
              <!-- Modal -->
